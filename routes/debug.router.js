...
const {
    ...
    regenerateUserLogoMany, playDebugMatch_v_17otr_v1,
    playPhaseMatches_debug, playWeekendLeagues_debug
...
DebugApiRouter.get('/regenerateAllPlayersAndLastTeams', async (req, res, next) => {
    try {
...
DebugApiRouter.get('/regenerateAllUserPlayersAndLastTeamWithRandomRatings', async (req, res, next) => {
    try {
        console.log('(routes/debug.router.js) Started regenerating all players with random ratings!');
        await regenerateUserPlayersAndLastTeamWithRandomRatingsMany();

        res.json({
            message: 'Все игроки и команды юзеров перегенерированы с рандомными рейтингами!'
        });
        console.log('(routes/debug.router.js) All players with random ratings was regenerated succesfully!');
    }
    catch (e) {
        next(e);
    }
...
// matches ===============================================================================
DebugApiRouter.get('/regenerate_matches_for_all_leagues', async (req, res, next) => {
    async function createLeagues() {
        ...
        lastLeagueUpdateTimestamp.setUTCDate(lastLeagueUpdateTimestamp.getUTCDate() - 14);
...
DebugApiRouter.get('/playAllMatchesTillNow', async (req, res, next) => {
    try {
        ...
        console.log(`(routes/debug.router.js) Now is ${new Date().toUTCString()}, starting playing all matches till now!`);
        for (let i = 0; i < matches.length; i++) {
            await playDebugMatch_v_17otr_v1(matches[i].id, true);
        }
        console.log(`(routes/debug.router.js) Played all matches till now! Count: ${matches.length}.`);
...
DebugApiRouter.get('/clear_weekend_tables', async (req, res, next) => {
    try {
        await prisma.$transaction([prisma.weekend_match.deleteMany({}), prisma.weekend_league_players.deleteMany({}), prisma.weekend_league.deleteMany({})]);
...
DebugApiRouter.post('/play_phase_weekend_matches', async (req, res, next) => {
    try {
        ...
        if (!Number.isInteger(phase) || (phase < 1 || phase > 10)) throw ApiError.BadRequest('Such phase doesn\'t exists!');
...
DebugApiRouter.get('/create_and_play_weekend_leagues', async (req, res, next) => {
    try {
