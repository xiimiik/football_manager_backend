...
const {getUTCDateZeroTimestamp, getNextNeededWeekDayTimestamp} = require("../utils/dates.utils");
    ...
    if (goals[0] === goals[1]) {
        ...
        for (let shotNum = 0; shotNum < 5; shotNum++) {
            ...
            if (goalkeeper.card.mood < -1) {
                if (shootingPlayer.card.mood < -1) goalChances = 0.80; //shPl = c
                else if (shootingPlayer.card.mood < 2) goalChances = 0.90; //shPl = b
                else goalChances = 0.95; //shPl = a
            }
            else if (goalkeeper.card.mood < 2) {
                if (shootingPlayer.card.mood < -1) goalChances = 0.70; //shPl = c
                else if (shootingPlayer.card.mood < 2) goalChances = 0.80; //shPl = b
                else goalChances = 0.90; //shPl = a
            }
            else {
                if (shootingPlayer.card.mood < -1) goalChances = 0.60; //shPl = c
                else if (shootingPlayer.card.mood < 2) goalChances = 0.70; //shPl = b
                else goalChances = 0.80; //shPl = a
            }
        ...
        //if penalty got draw
        let shotPlayer1Idx = 0, shotPlayer2Idx = 0;
...
async function createLeaguesNMatches() {
    ...
    for (let servIdx = 0; servIdx < servers.length; servIdx++) {
        for (let cntrIdx = 0; cntrIdx < servers[servIdx].region.countries.length; cntrIdx++) {
            for (let lgIdx = 0; lgIdx < servers[servIdx].region.countries[cntrIdx].leagues.length; lgIdx++) {
                let currLeague = servers[servIdx].region.countries[cntrIdx].leagues[lgIdx];

                // creation of first phase matches ==================================================
                let matchesData = [];
                for (let usIdx = 0; usIdx < currLeague.leaguePlayers.length / 2; usIdx++) {
                    matchesData.push({
                        user1_id: currLeague.leaguePlayers[usIdx].playerId,
                        user2_id: currLeague.leaguePlayers[currLeague.leaguePlayers.length - 1 - usIdx].playerId,
                        time: nextTimestamp
                    })
                }
                // creation of first phase matches ==================================================
                ...
                leaguesQueries.push(prisma.weekend_league.create({
                    data: {
                        isFull: false,
...
async function playMatchesPhase(phase) {
    console.log(`(weekend_leagues.js) Started playing weekend matches (phase ${phase})!`);
...
async function calcMatchesPhase(phase) {
    console.log(`(weekend_leagues.js) Started calculating weekend matches results (phase ${phase})!`);
    ...
    for (const league of leagues) {
        ...
        for (const currMatch of league.weekendLeagueMatches) {
            ...
            if (goals[0] > goals[1]) winnedUsers.push({
                playerId: currMatch.user1_id
            });
            else winnedUsers.push({
        ...
        for (let usIdx = 0; usIdx < winnedUsers.length / 2; usIdx++) matchesData.push({
...
async function createWeekendLeagues_v2(saturdayTimestamp) {
    ...
    for (let phaseIdx = 0; phaseIdx < 10; phaseIdx++) {
        let nextTimestamp = new Date(Date.UTC(
