...
async function scheduleLeaguesInterval() {
    async function createLeagues() {
        let dataBuffer = await fileHandle.readFile('./data/variables.txt'),
        ...
        for (let leagueIdx = 0; leagueIdx < allLeagues.length; leagueIdx++) {
            ...
            let firstTeams = [], secondTeams = [];
            for (let i = currLeague.leaguePlayers.length / 2 - 1; i >= 0; i--) firstTeams.push(currLeague.leaguePlayers[i].playerId);
            for (let i = currLeague.leaguePlayers.length / 2; i < currLeague.leaguePlayers.length; i++) secondTeams.push(currLeague.leaguePlayers[i].playerId);
            ...
            for (let i = 0; i < 5; i++) {
                for (let j = 0; j < 3; j++) {
                    ...
                    resultTimeWeek1.setUTCDate(lastLeagueUpdateTimestamp.getUTCDate() + 15 + i);
                    resultTimeWeek1.setUTCHours(utcHours[j], 0, 0, 0);
                    resultTimeWeek2.setUTCDate(lastLeagueUpdateTimestamp.getUTCDate() + 22 + i);
        ...
        for (let idx = 0; idx < userPlayers.length; idx++) {
            ...
            for (let plIdx = 0; plIdx < userPlayers[idx].playersJson.length; plIdx++) {
                if (userPlayers[idx].playersJson[plIdx].playerId > largestPlayerId) largestPlayerId = userPlayers[idx].playersJson[plIdx].playerId;
            }

            while (userPlayers[idx].playersJson.length < 20) {
                let generatedCard = generateCard(
                    largestPlayerId, false, userPlayers[idx].user.avatar, {currentAbility: -1, potential: -1},
            ...
            userPlayers[idx].playersJson = JSON.stringify(userPlayers[idx].playersJson);
        }

        await prisma.$transaction(
            userPlayers.map(
                user => (prisma.user_players.update({
                    data: {
    ...
    let
        dataBuffer = await fileHandle.readFile('./data/variables.txt'),
        data = JSON.parse(dataBuffer.toString('utf8')),
        lastLeagueUpdateTimestamp = new Date(data.lastLeagueUpdateTimestamp),
        now = new Date(),
    ...
    if (now.getTime() < lastLeagueUpdateTimestamp.getTime() + twoWeeksMs) {
        ...
        console.log(`(modules/leagues.js) All's fine, next leagues update at ${new Date(lastLeagueUpdateTimestamp.getTime() + twoWeeksMs).toUTCString()}. Days to update: ${(twoWeeksMs - (now.getTime() - lastLeagueUpdateTimestamp.getTime())) / (1000 * 60 * 60 * 24)}`);
    }
    else {
        setTimeout(() => {
            setInterval(() => {
        ...
        data.lastLeagueUpdateTimestamp = new Date(lastLeagueUpdateTimestamp.getTime() + twoWeeksMs);
        await fileHandle.writeFile('./data/variables.txt', JSON.stringify(data));

        console.log(`(modules/leagues.js) Its late, but leagues was updated (now lastLeagueUpdateTimestamp=${data.lastLeagueUpdateTimestamp})!`);
    }
}
...
async function scheduleMatchesInterval() {
        ...
        if (matchesCount > chunkLength) {
            for (let i = 0; i < Math.floor(matchesCount / chunkLength) + 1; i++) {
                let skip = i * chunkLength,
                    take = matchesCount - (i * chunkLength) >= chunkLength ? chunkLength : matchesCount - (i * chunkLength),
    ...
    try {
        ...
        let now = new Date(),
            ...
            msPassed = 1000 * 60 * 60 * ((now.getHours() % 8 + 5) % 8) + 1000 * 60 * now.getMinutes() + 1000 * now.getSeconds() + now.getMilliseconds(),
        ...
        console.log('(modules/leagues.js) Error at async scheduleMatchesInterval...', e);
    }
}
