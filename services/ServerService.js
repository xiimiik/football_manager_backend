...
class ServerService {
    async generateServer(regionId) {
        try {
            ...
            if (!regionCountries.length) throw ApiError.BadRequest(`Регион #${regionId} не существует!`);
            ...
            let fileData = await fileHandle.readFile(path.resolve(__dirname, '../data/bots-names-patterns.txt')),
                countryNamePatterns = JSON.parse(fileData);

            let botFullname = {}, botLogo;
                ...
                for (let j = 0; j < currCountry.leagues.length; j++) {
                    ...
                    if (patternsCountry) {
                        for (let k = 0; k < patternsCountry.leagues.length; k++) {
                            ...
                            if (currLeague.level === patternLeague.level || patternLeague === patternsCountry.leagues[patternsCountry.leagues.length - 1]) {
                                if (patternLeague.isPrimary) {
                                    for (let botI = 0; botI < 16; botI++) {
                                        botFullname.name = patternLeague.cities[botI] + ' ' + patternLeague.names[botI];
                                        botFullname.abbr = botFullname.name.substring(0, 4).toUpperCase();
                                        ...
                                        generatedBots.push(generateBot(botFullname, botLogo, currLeague.level));
                                    }
                                }
                                else {
                                    for (let botI = 0; botI < 16; botI++) {
                                        let city = patternLeague.cities[MathService.randomInteger(0, patternLeague.cities.length - 1)],
                                            nickname = patternLeague.nicknames[MathService.randomInteger(0, patternLeague.nicknames.length - 1)],
                                            name = patternLeague.names[MathService.randomInteger(0, patternLeague.names.length - 1)];

                                        botFullname.name = city + ' ' + nickname + ' ' + name;
                                        botFullname.abbr = city.substring(0, 1).toUpperCase() + nickname.substring(0, 1).toUpperCase() + name.substring(0, 1).toUpperCase();
                                        botLogo = null;
                        ...
                        for (let botIdx = 0; botIdx < 16; botIdx++) generatedBots.push(generateBot(null, null, currLeague.level));
                    }
            ...
            for (let countryIdx = 0; countryIdx < regionCountries.length; countryIdx++) {
    ...
    async getLeaguesInServerCountry(serverId, countryId) {
        return await prisma.league.findMany({
            where: {
                server_id: serverId, country_id: countryId,
            }, select: {
                id: true, isFull: true, level: true,
            }
        });
    }
    ...
    async getLeagueUsers(leagueId) {
        return await prisma.league_players.findMany({
            where: {
                ...
                player: {
                    select: {
                        id: true, name: true, logo: true, isBot: true,
                    }
                }
            }
        });
