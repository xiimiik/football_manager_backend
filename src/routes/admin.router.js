...
AdminApiRouter.get(
  "/game_server/:server_id/country/:country_id/leagues",
  async (req, res, next) => {
    try {
      ...
      if (!countryLeagues.length) {
        throw ApiError.BadRequest(
          "Лиги такого сервера и страны не были найдены..."
        );
      }
...
AdminApiRouter.post("/matchResults", async (req, res) => {
  ...
  console.log(
    `(routes/admin.router.js) Playing match #${matchId} ${count} times took ${
      new Date() - startMs
    } ms...\n==============================================`
  );
    ...
    for (let rngIdx = 0; rngIdx < valueRanges.length; rngIdx++) {
      ...
      for (let countryIdx = 0; countryIdx < countries.length; countryIdx++) {
        if (countries[countryIdx].name === country) {
          for (
            let leagueIdx = 0;
            leagueIdx < countries[countryIdx].leagues.length;
            leagueIdx++
          ) {
            if (
              countries[countryIdx].leagues[leagueIdx].level === leagueLevel
            ) {
      ...
      for (let rowIdx = 2; rowIdx < currRange.values.length; rowIdx++) {
        if (currLeague.isPrimary) {
          ...
          if (currRange.values[rowIdx][0])
            currLeague.cities.push(currRange.values[rowIdx][0]);
          if (currRange.values[rowIdx][1])
            currLeague.names.push(currRange.values[rowIdx][1]);
          if (currRange.values[rowIdx][2])
            currLeague.nicknames.push(currRange.values[rowIdx][2]);
          if (currRange.values[rowIdx][3])
            currLeague.colors.push(currRange.values[rowIdx][3]);
          if (currRange.values[rowIdx][4])
            currLeague.regions.push(currRange.values[rowIdx][4]);
        }
      }
    }
